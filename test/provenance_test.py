...
class TestOCDMProvenance:
    ...
    def test_generate_provenance_creation_no_snapshot_modification_ocdm_graph(self):
        ...
        assert (
            f"The entity '{self.subject}' has been created." == se_a.get_description()
        )
        ...
        assert ocdm_graph.provenance.counter_handler.prov_counters == {
            self.subject: 2,
            "https://w3id.org/oc/meta/br/0636066666": 1,
        }

    def test_generate_provenance_modification_ocdm_conjunctive_graph_filesystem_counter(
        self,
    ):
        ...
        with open(
            ...
            assert json.load(outfile) == {
                "https://w3id.org/oc/meta/br/0605": 2,
                "https://w3id.org/oc/meta/br/0636066666": 1,
                "https://w3id.org/oc/meta/id/0636064270": 1,
                "https://w3id.org/oc/meta/id/0605": 1,
            }

    def test_generate_provenance_modification_ocdm_conjunctive_graph_database_counter(
        self,
    ):
        ...
        try:
            ocdm_conjunctive_graph = OCDMDataset(counter_handler=counter_handler)
            ocdm_conjunctive_graph.parse(os.path.join("test", "br.nq"))
            ocdm_conjunctive_graph.provenance.counter_handler.set_counter(
                1, self.subject
            )
            ocdm_conjunctive_graph.preexisting_finished()
            ocdm_conjunctive_graph.provenance.counter_handler.set_counter(
            ...
            assert se_a_2 is not None
            assert (
                se_a_2.get_description() == f"The entity '{self.subject}' was modified."
            )
            assert se_a_2.get_is_snapshot_of() == URIRef(self.subject)
            assert se_a_2.get_derives_from()[0].res == URIRef(
                "https://w3id.org/oc/meta/br/0605/prov/se/1"
    ...
    def test_generate_provenance_after_merge(self):
        ...
        ocdm_conjunctive_graph.parse(os.path.join("test", "br.nq"))
        ocdm_conjunctive_graph.preexisting_finished()
        ocdm_conjunctive_graph.merge(
            URIRef("https://w3id.org/oc/meta/id/0605"),
            URIRef("https://w3id.org/oc/meta/id/0636064270"),
        )
        ocdm_conjunctive_graph.generate_provenance()
        se_a_2 = ocdm_conjunctive_graph.get_entity(
        ...
        assert {str(se.res) for se in se_a_2.get_derives_from()} == {
            ...
            "https://w3id.org/oc/meta/id/0636064270/prov/se/1",
        }
        se_b_1 = ocdm_conjunctive_graph.get_entity(
            "https://w3id.org/oc/meta/id/0636064270/prov/se/1"
        )
        assert se_b_1 is not None
        assert (
            se_b_1.get_description()
            == "The entity 'https://w3id.org/oc/meta/id/0636064270' has been created."
        )
        se_br_1 = ocdm_conjunctive_graph.get_entity(
            "https://w3id.org/oc/meta/br/0636066666/prov/se/1"
        )
        assert se_br_1 is not None
        assert (
            se_br_1.get_description()
            == "The entity 'https://w3id.org/oc/meta/br/0636066666' has been created."
        )
        se_br_2 = ocdm_conjunctive_graph.get_entity(
            "https://w3id.org/oc/meta/br/0636066666/prov/se/2"
        )
        ...
        se_id_0636064270_1 = ocdm_conjunctive_graph.get_entity(
            "https://w3id.org/oc/meta/id/0636064270/prov/se/1"
        )
        assert se_id_0636064270_1 is not None
        se_id_0636064270_2 = ocdm_conjunctive_graph.get_entity(
            "https://w3id.org/oc/meta/id/0636064270/prov/se/2"
        )
        assert se_id_0636064270_2 is not None
        assert (
            se_id_0636064270_1.get_description()
            == "The entity 'https://w3id.org/oc/meta/id/0636064270' has been created."
        )
        assert (
    ...
    def test_restore_deleted_entity(self):
        ...
        assert (
            initial_snapshot.get_description()
            == f"The entity '{self.subject}' has been created."
        )
        ...
        assert (
            deletion_snapshot.get_description()
            == f"The entity '{self.subject}' has been deleted."
        )
        ...
        assert (
            restore_snapshot.get_description()
            == f"The entity '{self.subject}' has been restored."
        )
    ...
    def test_entity_deleted_after_mark_with_graph_context(self):
        ...
        assert ocdm_dataset.entity_index[entity_uri]["graph_iri"] == URIRef(
        ...
        assert (
            deletion_snapshot.get_description()
            == f"The entity '{entity_uri}' has been deleted."
        )
