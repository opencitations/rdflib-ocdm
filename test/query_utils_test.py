...
from rdflib_ocdm.query_utils import get_delete_query, get_insert_query, get_update_query
...
class TestQueryUtils:
    ...
    def test_get_update_query_with_graph_type(self):
        ...
        query, added, removed = get_update_query(
            ocdm_graph, entity, entity_type="graph"
        )
        assert "INSERT DATA" in query
        assert added == 1
        assert removed == 0
    ...
    def test_get_update_query_with_prov_type(self):
        ...
        dataset.add(
            (
                ...
                Graph(identifier=URIRef("http://example.org/prov/")),
            )
        )
    ...
    def test_get_update_query_with_blank_node_context(self):
        ...
        dataset.add(
            (
                ...
                URIRef("http://example.org/p"),
                Literal("test"),
                Graph(identifier=blank_context),
            )
        )
