...
class TestReader:
    ...
    def setup(self):
        ...
        self.res_list = [
            URIRef("http://example.org/res1"),
            URIRef("http://example.org/res2"),
        ]
    ...
    def test_import_entities_ocdm_graph_success(self, mock_sparql):
        ...
        mock_sparql.return_value.queryAndConvert.return_value = Graph().parse(
            data=mock_response, format="xml"
        )

        ocdm_graph = OCDMGraph()
        self.reader.import_entities_from_triplestore(
            ocdm_graph, self.ts_url, self.res_list
        )
    ...
    def test_import_entities_conjunctive_graph_success(self, mock_sparql):
            ...
            "results": {
                "bindings": [
                    ...
                    },
                    {
                        ...
                        "o": {
                            "value": "A description with language",
                            "type": "literal",
                        },
                        "lang": {"value": "en"},
                    },
        ...
        for expected in expected_data:
            matching_quads = [
                ...
                if (
                    str(q[0]) == str(expected["s"])
                    and str(q[1]) == str(expected["p"])
                    and str(q[2]) == str(expected["o"])
                )
            ]
                ...
                "bindings": [
                    {
                        ...
                        "o": {
                            "value": "http://example.org/other-resource",
                            "type": "uri",
                        },
                    }
                ]
    ...
    def test_import_entities_retry_mechanism(self, mock_sparql):
        ...
        ocdm_graph = OCDMGraph()
        self.reader.import_entities_from_triplestore(
            ocdm_graph, self.ts_url, self.res_list, max_retries=2
        )
